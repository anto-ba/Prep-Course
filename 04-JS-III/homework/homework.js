...
function dePalabrasAFrase(palabras) {
  ...
  for(i=0; i<palabras.length; i++)
  {
    if(i==0)
    {
      frase[i]= palabras[i].concat(espacio);
        }
    else
       frase[i]= espacio.concat(palabras[i]) ;
  }
  return (frase);*/
}
...
function promedioResultadosTest(resultadosTest) {
  ...
  {
    sumresul += resultadosTest[i]; 
  }
  promedio = (sumresul/resultadosTest.length);
  return promedio;
}
...
function diaDeLaSemana(numeroDeDia) {
  ...
  //Escribe tu código aquí   
  //1dom,2lun,3mar,4mier,5jue,6vier,7sab.
